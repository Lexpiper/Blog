...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try{
      ...
      setloading(true)
      let res = await axios.post("http://172.20.10.2:3000/auth/signup", {
        email,
        password,
      },)
      ...
      <div className="flex shadow-sm rounded-md justify-center w-[65%] mx-auto h-[500px] ">
        <div className="flex bg-white flex-[1] flex-col items-center justify-center ">
          ...
          <form className="flex flex-col mt-[20px] w-[80%] ">
            {error ? <div className="text-[10px] font-italic text-red-400">
              {error} 
            </div> : null}
            ...
            <div className="flex items-center justify-between mt-[12px] w-full">
              ...
              <a className="text-[12px] cursor-pointer font-semibold text-zinc-600">Forget password?</a>
            </div>
            <button onClick={handleLogin} className="p-[5px] text-sm font-semibold bg-teal-300 rounded-md mt-[12px] h-[40px] hover:bg-teal-300 transition ease-in-out delay-150">
            ...
            <button className="ml-[5px] mt-4 border w-full p-1 h-[40px] rounded-md hover:border-black hover:border-1 transition ease-in-out delay-150 flex flex-row items-center justify-center">
             ...
             <img src={GoogleImg} alt={"Google image"} width={"100%"} height={"100%"} loading="lazy" />
             </div>
